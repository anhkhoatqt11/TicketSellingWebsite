...
const Coupon = ({ EventDetail, buyList, currentDateTime, setIsCouponUsing, setDiscountPrice, setCouponId }) => {
        ...
        const updatedBuyList = buyList.map((item) => {
          if (item.ticketId === couponItemId) {
            ...
            if (res[0].phanTramGiam) {
              discountedPrice = item.price - (item.price * (res[0].phanTramGiam || 0) / 100);
  ...
  if (couponUsed === true) return (
    <div className='flex flex-row justify-between'>
      <div className='flex flex-row'>
        <TicketIcon className='text-2xl text-default-400 pointer-events-none flex-shrink-0 mt-1' size={20} />
        <p className='font-bold text-lg ml-2'> {couponInput}</p>
      </div>
      <div className='cursor-pointer' onClick={() => {
        removeCoupon();
      }}>
        <XIcon className='text-2xl text-default-400 pointer-events-none flex-shrink-0 mt-2' size={20} />
      </div>
  ...
  return (
    <div className='grid grid-cols-2 gap-3'>
      <Input type="text"
        ...
        startContent={
          <TicketIcon className='text-2xl text-default-400 pointer-events-none flex-shrink-0' size={20} />
        } />
